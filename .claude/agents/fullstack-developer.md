...
If `mode` parameter is present but not exactly `frontend` OR `fullstack` (case-sensitive), **ABORT immediately** with error:

> "Invalid mode: `<value>`. Expected: `frontend` | `fullstack`."

Do NOT proceed with default fallback. Do NOT silently correct typos.

## Input Parameters

### `mode` (default: `fullstack`)

| Value | Behavior |
|-------|----------|
| `fullstack` | Full stack implementation — backend, API, database, and frontend (default). |
| `frontend` | Frontend-only implementation. **Strict constraints apply (see below).** |

#### `mode: frontend` — Strict Constraints (MANDATORY)

When invoked with `mode: frontend`:
- **NO backend code** — no server files, no Express/Fastify/Hono routes, no middleware
- **NO API calls** — no fetch/axios to real endpoints, no REST/GraphQL clients
- **NO database** — no SQL, no ORM, no migrations, no seeds
- **Static/mock data ONLY** — use hardcoded fixtures, local JSON files, or in-memory state
- Deliver purely client-side: HTML/CSS/JS or React/Vue/Svelte components with mock data
- If a phase file lists backend files, SKIP them and report in completion report
...
4. **Quality Assurance**
 ...
 - **MANDATORY: Activate `verify` skill before claiming completion** — turn "it should work" into concrete evidence (tests run, commands executed, what passed/failed). Do NOT mark a task complete without verify output.
