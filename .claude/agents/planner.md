...
## Rolling-Wave Planning Mode (Optional)

Activate when explicitly requested (`/plan --rolling` or when sprint scope is uncertain).

### Rules
- **Sprint 1**: Full detailed plan — all phases, tasks, file ownership, acceptance criteria, effort estimates
- **Sprint 2+**: Outline only — epic title, brief description, rough effort (no task breakdown)
- **Expand on trigger**: Expand Sprint N+1 to full detail only when Sprint N ships (or lead requests expansion)
- This mode is **opt-in**, NOT the default. Standard planning always produces full detail for all phases.
- **Sprint N+1 dep_files validated against Sprint N output before promoting outline to detail.** Fan-out pre-check MUST assert `dep_files` exist before spawning sub-agents.

### DAG Cycle Detection (MANDATORY)

Before emitting `dep-graph.md` in rolling-wave mode, validate the epic dependency graph has no cycles:

> If cycle detected (e.g., A → B → A) → **ABORT** with error listing the full cycle path. Ask user to break the cycle before proceeding.

Use DFS/topological-sort logic mentally: if any epic is reachable from itself through its dependencies, that is a cycle.

### Rolling-Wave Plan Structure

```
plans/{date}-{slug}/
├── plan.md                    # Overview with sprint statuses
├── sprint-01/                 # Full detail
│   ├── phase-01-*.md
│   └── phase-02-*.md
├── sprint-02-outline.md       # Outline only (expand after Sprint 1 ships)
└── sprint-03-outline.md       # Outline only
```

### Sprint Outline Format (Sprint 2+)

```markdown
## Sprint N — {Title} (Outline — expand after Sprint N-1 ships)

**Epics:** {list}
**Rough effort:** {Xd}
**Dependencies:** {Sprint N-1 deliverables}
**Expand trigger:** Sprint N-1 marked complete
```

### Sprint Outline File Format (`sprint-02-outline.md`)
...
goal: "..."
dep_files:
  - prototypes/<slug>/src/components/ui/Button.tsx
  - prototypes/<slug>/src/lib/data/seed.ts
status: outline
```

### Epic `complexity` Field (REQUIRED for team-vibe model tiering)

Every epic in rolling-wave plans MUST include `complexity` so team-vibe can dispatch to the right model:

```yaml
epics:
  - id: epic-1
    title: "..."
    complexity: simple   # required: simple | medium | complex
    dep_files: [...]
```
...
| Value | When to use |
|---|---|
| `simple` | Single file, no new deps, UI polish, bug fix |
| `medium` | 3-5 files, existing pattern reuse, CRUD endpoint |
| `complex` | New pattern, cross-module, auth/payment/migration, performance-critical |

If `complexity` is omitted → validation error before plan is emitted.
