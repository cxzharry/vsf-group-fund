...
6. **`shadcn-ui-design`** - shadcn/ui components, Tailwind CSS, design tokens (replaces legacy `ui-styling` ref)

**Before any design work**, run `ui-ux-pro-max` searches (prefers local skill copy, falls back to global):
```bash
SKILL=$(test -f ./.claude/skills/ui-ux-pro-max/scripts/search.py && echo ./.claude/skills/ui-ux-pro-max || echo $HOME/.claude/skills/ui-ux-pro-max)
python3 $SKILL/scripts/search.py "<product-type>" --domain product
python3 $SKILL/scripts/search.py "<style-keywords>" --domain style
python3 $SKILL/scripts/search.py "<mood>" --domain typography
python3 $SKILL/scripts/search.py "<industry>" --domain color
```
...
## Primary Deliverable (MANDATORY when called via `/team-design`)

**`designs/<slug>-design.md`** — markdown design specification, source of truth. Template at `templates/design-spec.md`. Must fill all 9 sections:

1. **Style Decision** — chosen style (from `templates/design-styles.md` menu OR ui-ux-pro-max search OR Custom "Other"), rationale, PO confirmation date, reference to `<slug>-style.md`
2. **Design Tokens** (human-readable: color, typography, spacing, radius, shadow, motion)
3. **Screen Inventory** — ≤7 MVP, one row per screen
4. **Component Inventory** — map to shadcn/ui primitives
5. **Per-Screen Specs** — layout, components, states, interactions, AC mapping
6. **User Flow** — numbered persona journey
7. **Dev Handoff Notes** — Tailwind config, shadcn deps, assets, a11y
8. **Implementation Reference** — `.pen` path, `tokens.json` path, prototype/app paths
9. **Revision Log**

### Derived outputs
- **`design-system/tokens.json`** — auto-derived from §2 of design.md (machine-readable subset). Consumers that want JSON read this; consumers that want context read design.md.
- **`designs/<slug>.pen`** — visual file (optional, when Pencil MCP available)
- **`designs/<slug>-style.md`** — style decision file (1-page companion to §1)

### Style picker (MANDATORY before any design work)

1. Read PRD §3.1 (persona), §4.2 (benefits), §4.3 (platform)
2. Load `templates/design-styles.md` catalog (12 curated styles with fit rules)
3. Augment with `ui-ux-pro-max` search (`python3 $SKILL/scripts/search.py "<style>" --domain style`)
4. Pick **top 3-5 candidates** matching product type
5. `AskUserQuestion` with 3-5 labels + auto `Other`, **top 1 marked Recommended** with 1-2 sentence rationale
6. On "Other" → follow-up AskUserQuestion for custom description/URL → map to nearest catalog entry
7. Save decision to `designs/<slug>-style.md`; design.md §1 inherits

**DO NOT silently auto-pick a style.** PO confirm is non-negotiable — aesthetic direction is PO decision, not agent judgment.
